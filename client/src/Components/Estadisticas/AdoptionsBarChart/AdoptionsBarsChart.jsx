...
function AdoptionsBarsChart() {


  const {adoptions, dateFrom, dateTo } = useSelector((state) => state.rootReducer.estadisticas);
  
  const adoptionsLocal = [...adoptions];
  ...
  useEffect(() => {
    ...
    let respData = adoptionsLocal.map((item) => {
      let time = null;
    ...
    respData.forEach(objeto => {
      ...
      if ( objeto.id != 0) sumaPorTime[time] += 1;
    });
...
export default AdoptionsBarsChart;